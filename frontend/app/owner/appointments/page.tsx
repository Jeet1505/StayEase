...
export default function OwnerAppointmentsPage() {
  ...
  const loadAppointments = async () => {
    try {
      setLoading(true)
      const data = await appointmentsAPI.getByOwner(user!.id)
  ...
  const handleStatusChange = async (id: number, status: "confirmed" | "cancelled") => {
    try {
      ...
      await loadAppointments() // Reload to get updated status from backend
    } catch (err) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Manage visit requests for your properties</p>
      </div>
              ...
              {appointments.length === 0 ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {appointments.map((apt) => (
                    <AppointmentCard key={apt.id} appointment={apt} onStatusChange={handleStatusChange} showActions />
                  ))}
                </div>
              )}
            </TabsContent>
            ...
            <TabsContent value="pending" className="mt-6">
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filterByStatus("pending").length === 0 ? (
                  ...
                  filterByStatus("pending").map((apt) => (
                    <AppointmentCard key={apt.id} appointment={apt} onStatusChange={handleStatusChange} showActions />
                  ))
                )}
              </div>
